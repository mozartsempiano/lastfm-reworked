...
function onPageChange() {
	try {
		...
		setTimeout(() => {
			if (
				currentSettings.hideDeletedShouts ||
				currentSettings.invertShoutActions
			) {
				reapplyShoutSettings();
			}
		}, 1000);
...
function hideNavItemByHref(hrefEndsWith, enabled) {
	document
		.querySelectorAll(`.secondary-nav a[href$="${hrefEndsWith}"]`)
		.forEach((a) => {
			if (a.closest("li"))
				a.closest("li").style.display = enabled ? "none" : "";
		});
	document
		.querySelectorAll(`.navlist-hidden-list a[href$="${hrefEndsWith}"]`)
		.forEach((a) => {
			if (a.closest("li"))
				a.closest("li").style.display = enabled ? "none" : "";
		});
...
function updateMoreButtonVisibility() {
	...
	const hasVisible = Array.from(hiddenList.querySelectorAll("li")).some(
...
function applyAllNavHides() {
	browserAPI.storage.local.get(
		...
		(localData) => {
			setHideNavReports(
				localData.hideNavReports !== null ? localData.hideNavReports : false
			);
			setHideNavPlaylists(
				localData.hideNavPlaylists !== null ? localData.hideNavPlaylists : false
			);
			setHideNavLoved(
				localData.hideNavLoved !== null ? localData.hideNavLoved : false
			);
			setHideNavObsessions(
				localData.hideNavObsessions !== null
					? localData.hideNavObsessions
					: false
			);
			setHideNavEvents(
				localData.hideNavEvents !== null ? localData.hideNavEvents : false
			);
			setHideNavNeighbours(
				localData.hideNavNeighbours !== null
					? localData.hideNavNeighbours
					: false
			);
			setHideNavTags(
				localData.hideNavTags !== null ? localData.hideNavTags : false
			);
			setHideNavShouts(
				localData.hideNavShouts !== null ? localData.hideNavShouts : false
			);
		}
	);
}
...
function reapplyShoutSettings() {
	...
	try {
		...
		setTimeout(() => {
			...
			if (
				currentSettings.hideDeletedShouts &&
				!document.body.classList.contains("hide-deleted-shouts")
			) {
				document.body.classList.add("hide-deleted-shouts");
				retryNeeded = true;
			}
			...
			if (
				currentSettings.invertShoutActions &&
				!document.body.classList.contains("invert-shout-actions")
			) {
				document.body.classList.add("invert-shout-actions");
				retryNeeded = true;
			}
		...
		setTimeout(() => {
			try {
				...
				console.warn(
					"LastFM Reworked: Fallback retry also failed:",
					fallbackError
				);
			}
		}, 200);
	...
	bodyClassObserver = new MutationObserver((mutations) => {
		mutations.forEach((mutation) => {
			if (
				...
				if (
					currentSettings.hideDeletedShouts &&
					!bodyClasses.contains("hide-deleted-shouts")
				) {
				...
				if (
					currentSettings.invertShoutActions &&
					!bodyClasses.contains("invert-shout-actions")
				) {
					needsReapplication = true;
				}
...
function applyAllHides() {
	browserAPI.storage.local.get(
		...
		(localData) => {
			setHidePlayButtons(
				localData.hidePlayButtons !== null ? localData.hidePlayButtons : true
			);
			setHideBuyButtons(
				localData.hideBuyButtons !== null ? localData.hideBuyButtons : true
			);
			setHideDetailsGrid(
				localData.hideDetailsGrid !== null ? localData.hideDetailsGrid : false
			);
			setHideSidebarLabs(
				localData.hideSidebarLabs !== null ? localData.hideSidebarLabs : true
			);
			setHideSidebarProgress(
				localData.hideSidebarProgress !== null
					? localData.hideSidebarProgress
					: true
			);
			setHideSidebarStations(
				localData.hideSidebarStations !== null
					? localData.hideSidebarStations
					: true
			);
			setHideUpsells(
				localData.hideUpsells !== null ? localData.hideUpsells : true
			);
		}
	);
}
...
browserAPI.runtime.onMessage.addListener((msg) => {
	...
	if (msg.type === "setHideSidebarProgress")
		setHideSidebarProgress(msg.enabled);
	if (msg.type === "setHideSidebarStations")
		setHideSidebarStations(msg.enabled);
...
function observeShoutUserChanges() {
	...
	function ensureCurrentlyPlayingSpan(
		shoutUserH3,
		{ artist, song, artistLink, songLink }
	) {
	...
	async function fetchCurrentlyPlaying(username) {
		try {
			...
			if (!profileResponse.ok) {
				throw new Error(
					`Erro ao acessar o perfil do usuário: ${profileResponse.status}`
				);
			}
			...
			const nowPlayingRow = profileDoc.querySelector(
				"tr.chartlist-row--now-scrobbling"
			);

			if (nowPlayingRow) {
				const artistElement = nowPlayingRow.querySelector(
					"td.chartlist-artist a"
				);
				const songElement = nowPlayingRow.querySelector(
					"td.chartlist-name a[href*='/_/']"
				);
			...
			console.error(
				`Erro ao buscar faixa atual para o usuário ${username}:`,
				error
			);
		}
		return null;
...
function applyAllSettings() {
	browserAPI.storage.local.get(
		...
		(localData) => {
			try {
				setRoundedBorders(
					localData.roundedBorders !== null ? localData.roundedBorders : true
				);
				setSquareAvatars(
					localData.squareAvatars !== null ? localData.squareAvatars : true
				);
				setHidePlayButtons(
					localData.hidePlayButtons !== null ? localData.hidePlayButtons : true
				);
				...
				setHideNavPlaylists(
					localData.hideNavPlaylists !== null
						? localData.hideNavPlaylists
						: false
				);
				setHideNavLoved(
					localData.hideNavLoved !== null ? localData.hideNavLoved : false
				...
				setHideActions(
					localData.hideActions !== null ? localData.hideActions : false
				);
				setLargerStats(
					localData.largerStats !== null ? localData.largerStats : false
				);
				setUseCustomFont(
					localData.useCustomFont !== null ? localData.useCustomFont : true
				);
				setCompactMode(
					localData.compactMode !== null ? localData.compactMode : true
				);
				setCompactTags(
					localData.compactTags !== null ? localData.compactTags : false
				);
				setCompactButtons(
					localData.compactButtons !== null ? localData.compactButtons : true
				);
				setMainColor(localData.mainColor || "default");

				// Apply shout settings and update current state
				const hideDeletedShouts =
					localData.hideDeletedShouts !== null
						? localData.hideDeletedShouts
						: false;
				const invertShoutActions =
					localData.invertShoutActions !== null
						? localData.invertShoutActions
						: true;
...
function handleCharacterCountColors() {
	try {
		const helpTexts = document.querySelectorAll(
			".form-row-help-text.js-field-help-text"
		);
		...
		helpTexts.forEach((helpText) => {
			...
			if (charCountSpan) {
				...
				if (charCount >= 1000) {
					...
					console.log(
						`LastFM Reworked: Character count at danger level: ${charCount}`
					);
				} else if (charCount >= 750) {
					helpText.classList.add("char-count-warning");
					console.log(
						`LastFM Reworked: Character count at warning level: ${charCount}`
					);
				}
		...
		console.warn(
			"LastFM Reworked: Error in handleCharacterCountColors:",
			error
		);
	}
}
...
function observeCharacterCount() {
	...
	const targets = [
		...document.querySelectorAll(
			'.shout-form, .js-shout-form, [class*="shout-form"]'
		),
	...
	if (targets.length > 0) {
		charCountObserver = new MutationObserver((mutations) => {
			...
			mutations.forEach((mutation) => {
					...
					if (
						...
						(mutation.addedNodes &&
							Array.from(mutation.addedNodes).some(
								(node) =>
									node.nodeType === 1 &&
									(node.classList?.contains("js-char-count") ||
										node.querySelector?.(".js-char-count"))
							))
					) {
						shouldUpdate = true;
					}
				...
				if (
					mutation.type === "attributes" &&
					(mutation.target.classList?.contains("js-char-count") ||
						mutation.target.closest?.(".form-row-help-text"))
				) {
					shouldUpdate = true;
				}
			});
...
function onPageChange() {
	try {
		...
		// Reaplica o sumiço do cabeçalho após navegação dinâmica
		setTimeout(() => {
...
function initializeExtension() {
	try {
		...
		console.warn(
			"LastFM Reworked: Error during extension initialization:",
			error
		);
	}
}
